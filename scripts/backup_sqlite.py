...
def backup(source:Path,destination:Path,retention:int):
    ...
    for old in files[retention:]:old.unlink()
    print(f"Backup selesai: {target}",flush=True)

if __name__=="__main__":
    p=argparse.ArgumentParser();p.add_argument("--source",type=Path,default=Path("data/processed/sebatik.db"));p.add_argument("--destination",type=Path,default=Path("backup"));p.add_argument("--retention",type=int,default=30);p.add_argument("--interval",type=int,default=86400);p.add_argument("--watch",action="store_true");a=p.parse_args()
    while True:
