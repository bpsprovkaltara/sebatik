...
def run(pdf_path: Path, db_path: Path, mapping_path: Path, report_path: Path, alignment_path: Path):
    ...
    start = next(i for i, (_, text) in enumerate(texts) if re.search(r"2\.3\.\s*Metadata", text, re.I) and "5 INDIKATOR SASARAN VISI DALAM RPJPD PROVINSI" in text)
    ...
    indicators = conn.execute("SELECT * FROM indikator ORDER BY CASE kategori WHEN 'ISV' THEN 0 ELSE 1 END, nomor").fetchall()
    conn.execute("UPDATE metadata_indikator SET definisi=NULL, rumus=NULL, rumus_mentah=NULL, interpretasi=NULL, sumber_data=NULL, frekuensi=NULL, halaman_sumber=NULL, perlu_verifikasi_manual=0, sumber_metadata=NULL, nama_di_buku1=NULL")
    card_names = [c.name for c in cards]
    mapping_rows = []
    # Pemetaan satu-ke-satu secara greedy berdasarkan skor tertinggi.
    pairs = sorted(((score_names(ind["nama_indikator"], name), i, j) for i, ind in enumerate(indicators) for j, name in enumerate(card_names)), reverse=True)
    assigned_ind, assigned_card, assignments = set(), set(), {}
    for score, i, j in pairs:
        if score < 85: break
        if i not in assigned_ind and j not in assigned_card:
            assignments[i] = (j, score); assigned_ind.add(i); assigned_card.add(j)
    for i, ind in enumerate(indicators):
        ...
        mapping_rows.append({"id_indikator": ind["id_indikator"], "nama_di_excel": ind["nama_indikator"], "nama_di_buku1": name or "", "skor": round(score, 2), "status_cocok": status})
        if status == "COCOK" and card_index is not None:
            card = cards[card_index]
            conn.execute("""UPDATE metadata_indikator SET definisi=?, rumus_mentah=?, interpretasi=?, sumber_data=?, frekuensi=?, halaman_sumber=?, perlu_verifikasi_manual=1, sumber_metadata='Buku 1 RPJPN-RPJPD 2025-2045', nama_di_buku1=? WHERE id_indikator=?""",
                         (card.definition, card.raw_formula, card.interpretation, card.source, card.frequency, str(card.page), card.name, ind["id_indikator"]))
            ...
            conn.execute("""UPDATE metadata_indikator SET definisi=?, sumber_data=?, frekuensi=?, perlu_verifikasi_manual=1, sumber_metadata='BPS Kaltara' WHERE id_indikator=?""",
                         (fallback, source or None, ind["periode_data"], ind["id_indikator"]))
    conn.commit()
    complete = conn.execute("SELECT COUNT(*) FROM metadata_indikator WHERE definisi IS NOT NULL AND rumus_mentah IS NOT NULL AND interpretasi IS NOT NULL AND sumber_data IS NOT NULL AND frekuensi IS NOT NULL").fetchone()[0]
    partial = conn.execute("SELECT COUNT(*) FROM metadata_indikator WHERE (definisi IS NOT NULL OR rumus_mentah IS NOT NULL OR interpretasi IS NOT NULL OR sumber_data IS NOT NULL OR frekuensi IS NOT NULL) AND NOT (definisi IS NOT NULL AND rumus_mentah IS NOT NULL AND interpretasi IS NOT NULL AND sumber_data IS NOT NULL AND frekuensi IS NOT NULL)").fetchone()[0]
    empty = 86 - complete - partial
    official = conn.execute("SELECT COUNT(*) FROM metadata_indikator WHERE sumber_metadata='Buku 1 RPJPN-RPJPD 2025-2045'").fetchone()[0]
    ...
    with mapping_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["id_indikator","nama_di_excel","nama_di_buku1","skor","status_cocok"]); writer.writeheader(); writer.writerows(mapping_rows)
    report = ["# Laporan Cakupan Metadata Buku 1", "", f"- Awal sub-bab 2.3 terdeteksi pada halaman PDF **{texts[start][0]}**.", f"- Kartu metadata berhasil diparse: **{len(cards)}**.", f"- Cocok otomatis (skor >= 85): **{official}** indikator.", f"- Fallback BPS Kaltara: **{local}** indikator.", f"- Metadata lengkap: **{complete}/86**.", f"- Metadata sebagian: **{partial}/86**.", f"- Metadata kosong: **{empty}/86**.", "", "Semua rumus hasil ekstraksi disimpan sebagai `rumus_mentah` dan ditandai `perlu_verifikasi_manual = TRUE`. Skor di bawah 85 tidak dipaksakan menjadi pasangan."]
    report_path.write_text("\n".join(report)+"\n", encoding="utf-8")
    print(f"Metadata: kartu={len(cards)}, resmi={official}, lokal={local}, lengkap={complete}, sebagian={partial}, kosong={empty}")


if __name__ == "__main__":
    p=argparse.ArgumentParser(); p.add_argument("pdf",type=Path); p.add_argument("--db",type=Path,default=Path("data/processed/sebatik.db")); p.add_argument("--mapping",type=Path,default=Path("docs/03-pemetaan-metadata.csv")); p.add_argument("--report",type=Path,default=Path("docs/03-metadata-report.md")); p.add_argument("--alignment",type=Path,default=Path("data/processed/penyelarasan_sebelum_2.3.txt")); a=p.parse_args()
