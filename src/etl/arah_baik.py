...
UP_PATTERNS = [
    (r"indeks|kontribusi|proporsi|persentase|persen|rasio|cakupan|akses|harapan|produktivitas|pertumbuhan|pendapatan|pdrb|nilai|peringkat", "ukuran capaian/akses/produktivitas diasumsikan membaik ketika naik"),
]
...
def ensure_columns(conn: sqlite3.Connection) -> None:
    ...
    if "arah_baik_terverifikasi" not in columns:
        conn.execute("ALTER TABLE indikator ADD COLUMN arah_baik_terverifikasi INTEGER NOT NULL DEFAULT 0 CHECK(arah_baik_terverifikasi IN (0,1))")
...
def run(db_path: Path, output_path: Path) -> None:
    ...
    ensure_columns(conn)
    indicators = conn.execute("SELECT id_indikator, nama_indikator, kategori, kelompok, arah_pembangunan FROM indikator ORDER BY CASE kategori WHEN 'ISV' THEN 0 ELSE 1 END, nomor").fetchall()
    rows = []
    for item in indicators:
        direction, keyword, reason = infer_direction(item["nama_indikator"])
        conn.execute("UPDATE indikator SET arah_baik=?, arah_baik_terverifikasi=0 WHERE id_indikator=?", (direction, item["id_indikator"]))
        rows.append({
...
if __name__ == "__main__":
    ...
    args = parser.parse_args(); run(args.db, args.output)
